...
// ═══════════════════════════════════════════════════════════════════
// Carvak.com.tr Adapter — Real scraping with graceful fallback
// ═══════════════════════════════════════════════════════════════════
...
export class CarvakAdapter extends BaseAdapter {
  sourceName = 'carvak';
  baseUrl = 'https://www.carvak.com.tr';
  defaultDelay = 2500;
  maxConcurrency = 2;
  ...
  async getDetail(listingId: string): Promise<ListingRaw | null> {
    try {
      ...
      $('.spec-list li, .detail-features li, .vehicle-specs li, .specs-item').each((_, el) => {
      ...
      $('.gallery img, .vehicle-photos img, .detail-images img, .carousel img').each((_, el) => {
      ...
      const listing: ListingRaw = {
        ...
        sellerType: 'Yetkili Bayi', // Carvak is a curated inspected platform
  ...
  /**
   * Build the Carvak search URL from filter parameters.
   * Maps our internal filter names to Carvak's query parameter names.
   */
  private buildSearchUrl(filters: SearchFilters): string {
    ...
    // Fuel type
    if (filters.fuelType) {
      params.set('yakit', this.slugify(filters.fuelType));
    }
  ...
  private extractListingFromCard(
    ...
    // ── Seller type hint ─────────────────────────────────────────
    const sellerHint = $el.find('.seller, .seller-type, .store').first().text().trim();
    ...
    if (sellerHint) {
      ...
      // Carvak is a curated platform — mostly Yetkili Bayi or Galeri
  ...
  /**
   * Extract the listing ID from a Carvak URL.
   * Carvak URLs may contain a numeric ID segment at the end.
   */
  private extractIdFromUrl(url: string): string | undefined {
    // Match a numeric ID at the end of the path
  ...
  /**
   * Slugify a string for use in Carvak URL paths.
