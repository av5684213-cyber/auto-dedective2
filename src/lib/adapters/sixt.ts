...
// ═══════════════════════════════════════════════════════════════════
// Sixt Adapter — Premium rental fleet sales (German brands focus)
// ═══════════════════════════════════════════════════════════════════
...
/** Fuel type query param mapping for Sixt */
...
/** Transmission query param mapping for Sixt */
...
export class SixtAdapter extends BaseAdapter {
  sourceName = 'sixt';
  ...
  async getDetail(listingId: string): Promise<ListingRaw | null> {
    try {
      ...
      // Title
      const title = $('h1, .vehicle-title, .car-title, .detail-title').first().text().trim();
      const { make, model } = this.extractMakeModelFromTitle(title);

      // Price
      const priceText = $('.price, .vehicle-price, .fleet-price, .car-price').first().text().trim();

      // Detail attributes from specification rows
      const details: Record<string, string> = {};
      $('.specifications li, .vehicle-details li, .detail-info li, .specs-row').each((_, el) => {
      ...
      // Description
      const description = $('.description, .vehicle-description, #description').text().trim();

      // Images
      const imageUrls: string[] = [];
      $('.vehicle-photos img, .gallery img, .carousel img, .detail-images img').each((_, el) => {
      ...
      const listing: ListingRaw = {
        ...
        sellerType: 'Galeri', // Sixt is always a gallery/dealer
  ...
  /**
   * Build the Sixt search URL from filter parameters.
   * Sixt uses query params: marka, model, minYil, maxYil, minFiyat, maxFiyat,
   ...
   * Tries multiple CSS selectors since Sixt's page structure may change.
   */
  ...
  private extractListingFromCard(
    ...
    const title =
      linkEl.attr('title') ||
      linkEl.text().trim() ||
      $el.find('.title, .car-title, .vehicle-title, h2, h3').first().text().trim();

    // ── Price ────────────────────────────────────────────────────
    const priceText =
      $el.find('.price, .vehicle-price, .fleet-price, .car-price, [data-price]').first().text().trim();
    ...
    const listing: ListingRaw = {
      ...
      sellerType: 'Galeri', // Sixt is always a gallery
      imageUrl,
      isActive: true,
    };
  ...
  /**
   * Extract the listing ID from a Sixt URL.
   * Sixt URLs may contain a numeric ID in the path or query.
   */
  private extractIdFromUrl(url: string): string | undefined {
    // Match a numeric ID in the path, e.g. /ikinci-el-araba/12345
   ...
   * Uses greedy matching (longest make name first).
   */
  ...
  /**
   * Slugify a string for use in Sixt URL query params.
   */
