...
// ═══════════════════════════════════════════════════════════════════
// Hasarlı Araba Adapter — Accident-damaged vehicles (even cheaper)
// ═══════════════════════════════════════════════════════════════════
...
/** Damage type descriptions specific to Hasarlı Araba */
const DAMAGE_TYPES = [
  'ön çarpma',
  'arka çarpma',
  'yan çarpma',
  ...
  'sel suyu hasarlı',
  'hasar kayıtlı',
  'trafik sigortası hasarlı',
];

export class HasarliArabaAdapter extends BaseAdapter {
  sourceName = 'hasarliaraba';
  baseUrl = 'https://www.hasarliaraba.com';
  defaultDelay = 3000;
  maxConcurrency = 2;
  ...
  async getDetail(listingId: string): Promise<ListingRaw | null> {
    try {
      const url = `${this.baseUrl}/hasarli-araba/${listingId}`;
      ...
      // Title
      const title = $('h1, .vehicle-title, .car-title, .damage-title, .detail-title').first().text().trim();
      const { make, model } = this.extractMakeModelFromTitle(title);

      // Price — Hasarlı Araba prices are the cheapest (10-35% of normal)
      const priceText = $('.price, .vehicle-price, .damage-price, .car-price').first().text().trim();
      ...
      // Description — contains damage details
      const description = $('.description, .vehicle-description, #description, .damage-description').text().trim();

      // Damage type from dedicated field or description
      const damageType = details['hasar tipi'] || details['hasar durumu'] || details['hasar'] || '';

      // Seller type — Hasarlı Araba has both gallery and individual sellers
      const sellerTypeHint = details['satıcı tipi'] || details['sahibi'] || '';

      // Images
      const imageUrls: string[] = [];
      $('.vehicle-photos img, .gallery img, .carousel img, .detail-images img, .damage-images img').each((_, el) => {
      ...
      const listing: ListingRaw = {
        id: listingId,
        sourceName: this.sourceName,
        sourceUrl: `${this.baseUrl}/hasarli-araba/${listingId}`,
        ...
        sellerType: sellerTypeHint ? this.normalizeSellerType(sellerTypeHint) : undefined,
        imageUrl: imageUrls[0] || undefined,
        imageUrls: imageUrls.length > 0 ? imageUrls : undefined,
        description: description || (damageType
          ? `${year} ${make || details['marka'] || ''} ${model || details['model'] || ''} hasar kayıtlı, ${damageType}. Detaylı bilgi için arayın.`
          : undefined),
        isActive: true,
      };
  ...
  /**
   * Build the Hasarlı Araba search URL from filter parameters.
  ...
  private buildSearchUrl(filters: SearchFilters): string {
    ...
    return `${this.baseUrl}/hasarli-araba${queryString ? '?' + queryString : ''}`;
  }

  /**
   * Parse search result HTML and extract listing cards.
   * Tries multiple CSS selectors since Hasarlı Araba's page structure may change.
   */
   ...
   * Hasarlı Araba cards include damage information and mixed seller types.
   */
  private extractListingFromCard(
    ...
    const title =
      linkEl.attr('title') ||
      linkEl.text().trim() ||
      $el.find('.title, .car-title, .damage-title, h2, h3').first().text().trim();

    // ── Price ────────────────────────────────────────────────────
    const priceText =
      $el.find('.price, .vehicle-price, .damage-price, [data-price]').first().text().trim();
    ...
    // ── Damage type ──────────────────────────────────────────────
    const damageText =
      $el.find('.damage, .damage-type, .hasar, [data-damage]').first().text().trim();

    // ── Seller type hint ─────────────────────────────────────────
    const sellerHint = $el.find('.seller, .seller-type, .owner, [data-seller]').first().text().trim();
    ...
    const description = damage
      ? `${year || ''} ${make} ${model} hasar kayıtlı, ${damage}. Detaylı bilgi için arayın.`
      : undefined;
  ...
  /**
   * Extract the listing ID from a Hasarlı Araba URL.
   */
  ...
  /**
   * Slugify a string for use in Hasarlı Araba URL query params.
   */
