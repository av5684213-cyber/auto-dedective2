...
// ═══════════════════════════════════════════════════════════════════
// Taşıt.com Adapter — General vehicle marketplace
// ═══════════════════════════════════════════════════════════════════
...
/** Fuel type query param mapping for Taşıt.com */
...
/** Transmission query param mapping for Taşıt.com */
...
export class TasitAdapter extends BaseAdapter {
  sourceName = 'tasit';
  baseUrl = 'https://www.tasit.com';
  defaultDelay = 2500;
  maxConcurrency = 3;
  ...
  async getDetail(listingId: string): Promise<ListingRaw | null> {
    try {
      ...
      // Title
      const title = $('h1, .vehicle-title, .car-title, .ad-title, .detail-title').first().text().trim();
      const { make, model } = this.extractMakeModelFromTitle(title);

      // Price
      const priceText = $('.price, .vehicle-price, .ad-price, .car-price').first().text().trim();

      // Detail attributes from specification rows
      const details: Record<string, string> = {};
      $('.specifications li, .vehicle-details li, .detail-info li, .info-row, .specs-row, table.detail-table td').each((_, el) => {
      ...
      // Description
      const description = $('.description, .vehicle-description, #description, .ad-description').text().trim();

      // Seller type — Taşıt.com has mixed sellers
      const sellerTypeHint = details['satıcı tipi'] || details['sahibi'] || details['satıcı'] || '';

      // Images
      const imageUrls: string[] = [];
      $('.vehicle-photos img, .gallery img, .carousel img, .detail-images img, .ad-images img').each((_, el) => {
      ...
      const listing: ListingRaw = {
        ...
        description: description || `${year} ${make || details['marka'] || ''} ${model || details['model'] || ''} ${city || ''}'da satılık. Detaylı bilgi ve fotoğraf için ilan sayfamızı ziyaret edin.`,
        isActive: true,
      };
  ...
  /**
   * Build the Taşıt.com search URL from filter parameters.
   * Uses query params: marka, model, minYil, maxYil, minFiyat, maxFiyat,
   * yakitTipi, vitesTipi, sayfa
   */
  private buildSearchUrl(filters: SearchFilters): string {
    ...
    return `${this.baseUrl}/ikinci-el-araba${queryString ? '?' + queryString : ''}`;
  }

  /**
   * Parse search result HTML and extract listing cards.
   * Tries multiple CSS selectors since Taşıt.com's page structure may change.
   */
  ...
  private extractListingFromCard(
    ...
    const title =
      linkEl.attr('title') ||
      linkEl.text().trim() ||
      $el.find('.title, .car-title, .vehicle-title, .ad-title, h2, h3').first().text().trim();

    // ── Price ────────────────────────────────────────────────────
    const priceText =
      $el.find('.price, .vehicle-price, .ad-price, [data-price]').first().text().trim();
    ...
    } else if ($el.find('.store-icon, .dealer-badge').length > 0) {
      sellerType = 'Galeri';
    }

    // Build description with city
    const description = cityText
      ? `${year || ''} ${make} ${model} ${cityText}'da satılık. Detaylı bilgi ve fotoğraf için ilan sayfamızı ziyaret edin.`
      : undefined;
  ...
  /**
   * Extract the listing ID from a Taşıt.com URL.
   */
  ...
  /**
   * Slugify a string for use in Taşıt.com URL query params.
   */
