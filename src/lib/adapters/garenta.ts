...
// ═══════════════════════════════════════════════════════════════════
// Garenta Adapter — Rental fleet sales platform (below market prices)
// Real scraping with graceful fallback
// ═══════════════════════════════════════════════════════════════════

/** Fuel type mapping: Turkish name → Garenta query param value */
...
/** Transmission mapping: Turkish name → Garenta query param value */
...
export class GarentaAdapter extends BaseAdapter {
  sourceName = 'garenta';
  baseUrl = 'https://www.garenta.com.tr';
  defaultDelay = 2500;
  maxConcurrency = 2;
  ...
  async getDetail(listingId: string): Promise<ListingRaw | null> {
    try {
      const url = `${this.baseUrl}/ikinci-el-arac/${listingId}/detay`;
      ...
      const listing: ListingRaw = {
        id: listingId,
        sourceName: this.sourceName,
        sourceUrl: `${this.baseUrl}/ikinci-el-arac/${listingId}`,
  ...
  async scrapeFallback(): Promise<ListingRaw[]> {
    ...
    return generateMockListings({
      ...
      descriptionTemplate: (make, model, year, _city) =>
        `${year} ${make} ${model} Garenta filo satış, düzenli bakımlı, sorunsuz araç.`,
    });
  }
  ...
  /**
   * Build the Garenta search URL from filter parameters.
   * Maps our internal filter names to Garenta's Turkish query parameter names.
   */
  private buildSearchUrl(filters: SearchFilters): string {
    const params = new URLSearchParams();

    // Base path for second-hand vehicles on Garenta
    let path = '/ikinci-el-arac';
  ...
  private extractListingFromCard(
    ...
    // Garenta is a fleet sales platform — all sellers are "Galeri"
    ...
    if (sellerHint) {
      ...
      // Garenta listings are always from their fleet sales gallery
      sellerType = 'Galeri';
    }
  ...
  /**
   * Extract the listing ID from a Garenta URL.
   * URLs typically contain a numeric ID, e.g. /ikinci-el-arac/.../12345
   */
  ...
  /**
   * Slugify a string for use in Garenta URL paths.
