...
// ═══════════════════════════════════════════════════════════════════
// Spoticar Adapter — PSA (Stellantis) certified pre-owned platform
// Real scraping with graceful fallback
// ═══════════════════════════════════════════════════════════════════

/** Fuel type mapping: Turkish name → Spoticar query param value */
...
/** Transmission mapping: Turkish name → Spoticar query param value */
...
export class SpoticarAdapter extends BaseAdapter {
  sourceName = 'spoticar';
  ...
  async getDetail(listingId: string): Promise<ListingRaw | null> {
    try {
      ...
      $('.vehicle-specs li, .detail-specs li, .spec-list li').each((_, el) => {
      ...
      $('.vehicle-gallery img, .detail-photos img, .carousel img').each((_, el) => {
  ...
  /**
   * Build the Spoticar search URL from filter parameters.
   * Maps our internal filter names to Spoticar's query parameter names.
   */
  private buildSearchUrl(filters: SearchFilters): string {
    const params = new URLSearchParams();

    // Base path for second-hand cars on Spoticar
    let path = '/ikinci-el-araba';

    // Make (brand) and model as path segments
    if (filters.make) {
  ...
  private extractListingFromCard(
    ...
    const title =
      linkEl.attr('title') ||
      linkEl.text().trim() ||
      $el.find('.car-title, .vehicle-title, .listing-title, .card-title').first().text().trim();

    // ── Price ────────────────────────────────────────────────────
    const priceText =
      $el.find('.price, .car-price, .vehicle-price, .listing-price, .certified-price').first().text().trim();
    ...
    // Spoticar is a certified pre-owned platform — all dealers are "Yetkili Bayi"
    const sellerHint = $el.find('.seller-type, .dealer-badge, .certified-badge').first().text().trim();

    let sellerType: string | undefined;
    if (sellerHint) {
      sellerType = this.normalizeSellerType(sellerHint);
    } else {
      // Spoticar listings are always from authorized dealers
      sellerType = 'Yetkili Bayi';
    }
  ...
  /**
   * Extract the listing ID from a Spoticar URL.
   * URLs typically contain a numeric or slug-based ID, e.g. /ikinci-el-araba/.../12345
   */
  ...
  /**
   * Slugify a string for use in Spoticar URL paths.
